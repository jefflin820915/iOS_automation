...
class TestGHAHome(BaseTestCase):
    def test_add_camera_device_and_oobe(self):
        ...
        gha_session.GHADevicePage.is_device_exist_device_page(self, device_name=self.device_name)
        gha_session.GHADevicePage.enter_device_page(self, device_name=self.device_name)
        gha_session.GHACameraLivePage.verify_camera_live_stream(self)
        logger.info("Start test case: remove device")
        gha_session.GHASession.handle_remove_device(self)
        gha_session.GHASession.refresh_gha_devices(self)
        assert gha_session.GHASession.stop_gha(self)
        time.sleep(10)
